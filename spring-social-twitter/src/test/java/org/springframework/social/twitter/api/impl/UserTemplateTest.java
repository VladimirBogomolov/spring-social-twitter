...
public class UserTemplateTest extends AbstractTwitterApiTest {
    ...
    @Test
    public void serializationTwitterProfileTest() throws Exception {
        mockServer.expect(requestTo("https://api.twitter.com/1.1/account/verify_credentials.json"))
                .andExpect(method(GET))
                .andRespond(withSuccess(jsonResource("twitter-profile"), APPLICATION_JSON));
        ...
        TwitterProfile profile = twitter.userOperations().getUserProfile();
        assertUser(profile);
        String profileJson = twitterObjectMapper.writeValueAsString(profile);
        TwitterProfile profile1 = twitterObjectMapper.readValue(profileJson, TwitterProfile.class);
        assertUser(profile1);
    }
}
