...
public class TimelineTemplateTest extends AbstractTwitterApiTest {
    ...
    @Test
    public void getStatus_withTickerSymbolEntity() throws IOException {
        mockServer.expect(requestTo("https://api.twitter.com/1.1/statuses/show/12345.json?include_entities=true"))
                .andExpect(method(GET))
        ...
        assertTickerEntity(tickerSymbols);
        ...
        String tweetJson = twitterObjectMapper.writeValueAsString(tweet);
        tweet = twitterObjectMapper.readValue(tweetJson, Tweet.class);
        entities = tweet.getEntities();
        tickerSymbols = entities.getTickerSymbols();
        assertTickerEntity(tickerSymbols);
    }

    @Test
    public void serializationTweet_withTickerSymbolEntity() {
        mockServer.expect(requestTo("https://api.twitter.com/1.1/statuses/show/12345.json?include_entities=true"))
                .andExpect(method(GET))
                .andRespond(withSuccess(jsonResource("status_with_tickers"), APPLICATION_JSON));
    ...
    private void assertTickerEntity(List<TickerSymbolEntity> tickerSymbols) {
